...
S3_BASE="s3://konnect-central-logs/dev/Konnect-DEV-Kibana/Daily"
...
echo "📅 Daily Kibana log upload"
echo "➡ Destination: $S3_DEST"

# Find rotated logs from last 24 hours
FILES=$(find "$LOG_DIR" -type f -name "*.gz" -mtime -1)

if [ -z "$FILES" ]; then
  echo "⚠️ No rotated Kibana logs found. Exiting."
  exit 0
fi
