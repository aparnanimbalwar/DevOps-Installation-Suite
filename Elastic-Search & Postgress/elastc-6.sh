...
############################################
# 1️⃣ FULL CLEANUP (ABSOLUTE RESET)
############################################
...
systemctl mask elasticsearch 2>/dev/null || true
...
############################################
# 5️⃣ INSTALL ELASTICSEARCH (NO AUTOSTART)
############################################
echo "📦 Installing Elasticsearch (masked)"
systemctl mask elasticsearch
apt install -y elasticsearch

############################################
# 6️⃣ REMOVE AUTO-SECURITY ARTIFACTS (CRITICAL)
############################################
echo "🧹 Cleaning Elastic auto-security artifacts"
...
echo "🧹 Cleaning Elastic auto-security keystore entries"

KEYSTORE="/etc/elasticsearch/elasticsearch.keystore"
if [ -f "$KEYSTORE" ]; then
  for key in $(/usr/share/elasticsearch/bin/elasticsearch-keystore list); do
    if [[ "$key" == xpack.security.* ]]; then
...
############################################
# 7️⃣ CONFIGURE ELASTICSEARCH (VALID 8.x)
############################################
...
############################################
# 8️⃣ START ELASTICSEARCH PROPERLY
############################################
...
############################################
# 9️⃣ RESET BUILT-IN PASSWORDS (NON-INTERACTIVE)
############################################
